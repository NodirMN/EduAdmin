...
const teacher = {
    ...
    actions: {
        ...
        axios.post(' http://localhost:3000/teachers', payload).then(response => {
            context.commit('addTeacher', response.data)
        })
        },
        allTeacher(context){
        ...
        deleteTeacher(context,index, id){
                axios.delete(' http://localhost:3000/teachers/' + id).then(response => {
                            if (response.status == 200) context.commit('delTeachers', index)
            })
        },
        updateTeacher(context,{index,teacher}){
            axios.put('http: //locolhost:300/teachers/'+teacher.id,teacher).then(response => 
            { const obj = {
                index: index,
